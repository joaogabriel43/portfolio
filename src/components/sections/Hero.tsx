...
function JavaCodeCard() {
  ...
  const Token = ({
    ...
    const colors: Record<TokenType, string> = {
      annotation:  "text-[#c9b97a]",
      keyword:     "text-[#c792ea]",
      type:        "text-[#82aaff]",
      identifier:  "text-[#f0ece4]",
      string:      "text-[#c3e88d]",
  ...
  const lines = [
    ...
    <><Token type="annotation">@Transactional</Token><Token type="plain">(readOnly = </Token><Token type="keyword">true</Token><Token type="plain">)</Token></>,
    <><Token type="keyword">public class </Token><Token type="identifier">PortfolioService </Token><Token type="operator">{"{"}</Token></>,
    <></>,
    <>{"  "}<Token type="keyword">private final </Token><Token type="type">ProjectRepository </Token><Token type="identifier">repo</Token><Token type="plain">;</Token></>,
    <></>,
    <>{"  "}<Token type="keyword">public </Token><Token type="type">List</Token><Token type="operator">{"<"}</Token><Token type="type">Project</Token><Token type="operator">{">"}</Token><Token type="method"> findFeatured</Token><Token type="operator">() {"{"}</Token></>,
    <>{"    "}<Token type="keyword">return </Token><Token type="identifier">repo</Token><Token type="operator">.</Token><Token type="method">findAll</Token><Token type="operator">()</Token></>,
    <>{"      "}<Token type="operator">.</Token><Token type="method">stream</Token><Token type="operator">()</Token></>,
    <>{"      "}<Token type="operator">.</Token><Token type="method">filter</Token><Token type="operator">(</Token><Token type="type">Project</Token><Token type="operator">::</Token><Token type="method">isFeatured</Token><Token type="operator">)</Token></>,
    <>{"      "}<Token type="operator">.</Token><Token type="method">sorted</Token><Token type="operator">(</Token><Token type="type">Comparator</Token></>,
    <>{"        "}<Token type="operator">.</Token><Token type="method">comparing</Token><Token type="operator">(</Token><Token type="type">Project</Token><Token type="operator">::</Token><Token type="method">getYear</Token><Token type="operator">)</Token></>,
    <>{"        "}<Token type="operator">.</Token><Token type="method">reversed</Token><Token type="operator">())</Token></>,
    <>{"      "}<Token type="operator">.</Token><Token type="method">collect</Token><Token type="operator">(</Token><Token type="method">toList</Token><Token type="operator">());</Token></>,
      ...
      >
        ...
        <div className="px-4 py-4 overflow-auto">
          <table className="w-full border-collapse">
            <tbody>
              {lines.map((line, i) => (
                <tr key={i} className={i === 6 ? "bg-accent/[0.04] rounded" : ""}>
...
export function Hero() {
  return (
    <section id="hero" className="relative min-h-screen flex items-center section-padding overflow-hidden">
      {/* ── Layer 1: Background decoratives (moves most) ── */}
      <Parallax3DLayer depth={1.8} className="absolute inset-0 pointer-events-none">
        <div className="hero-lines absolute inset-0 opacity-[0.04]" aria-hidden />
        <div
      ...
      <div className="container-main w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          {/* ── Layer 2: Text content (baseline depth) ── */}
          <Parallax3DLayer depth={1}>
          ...
          {/* ── Layer 3: Code card (foreground — moves least) ── */}
          <Parallax3DLayer depth={0.5} className="hidden lg:flex justify-center items-center">
            <motion.div
